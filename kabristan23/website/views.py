...
def add_book(request):
    ...
    if request.method == "POST":
        ...
        Book.objects.create(author=author,name=name,description=description)
        if form.is_valid():
            form.save()
    
    bookmd=Book.objects.all()
    context = {'form':form,'bookmd':bookmd}
    return render(request,"front_end/Book/add-book.html",context)

def list_book(request):
    books = Book.objects.all()
    context = {'books':books}
    return render(request,"front_end/Book/list-book.html",context)

def update_book(request,id):
    book = Book.objects.get(id=id)
    form = BookForm(initial={'name': book.name, 'description': book.description, 'author': book.author})
    if request.method == "POST":  
    ...
    return render(request,'front_end/Book/update-book.html',{'form':form}) 
